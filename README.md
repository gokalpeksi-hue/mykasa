...
- 📄 **PDF rapor**: görünen ayın gelir ve gider tabloları + yıllık aylık özet tablosu tek PDF olarak indirilir (çevrimdışı da çalışır).
- 🔒 **Veri koruması**: kalıcı depolama izni (`navigator.storage.persist`) istenir ve tüm kayıtların IndexedDB'de ayna kopyası tutulur — tarayıcı `localStorage`'ı temizlese bile eski ayların verileri otomatik geri gelir.
